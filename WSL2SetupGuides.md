...
Setting up Conda + Snakemake
----------------------------

1. Open up your Ubuntu Terminal, and install `conda`/`mamba` by running the following code and following the onscreen instructions. Make sure you select (Y)es when it asks you if you want to add conda activation to your shell:
```
curl -L -O "https://github.com/conda-forge/miniforge/releases/latest/download/Miniforge3-$(uname)-$(uname -m).sh"
bash Miniforge3-$(uname)-$(uname -m).sh
```
2. Install `Snakemake` in its own Conda environment by running:
```
mamba create -c conda-forge -c bioconda -c nodefaults -n snakemake snakemake
```
Now you can run `mamba activate snakemake`
...
sudo apt update
sudo apt install -y software-properties-common ca-certificates apt-transport-https gnupg dirmngr libharfbuzz-dev libfribidi-dev libgit2-dev gdebi-core pandoc texlive libcurl4-openssl-dev libxml2-dev libfontconfig1-dev libfreetype6-dev libpng-dev libtiff5-dev libjpeg-dev libwebp-dev make cmake g++ libblas-dev liblapack-dev gfortran libx11-dev libcairo2-dev
```
...
4. Install R by following the instructions here: [https://lib.stat.cmu.edu/R/CRAN/](https://lib.stat.cmu.edu/R/CRAN/). Important: make sure that you're following the instructions for your specific OS (Ubuntu/Debian if you're following this guide).
5. Install RStudio Server by following the instructions here: [https://posit.co/download/rstudio-server/](https://posit.co/download/rstudio-server/). Important: make sure that you're following the instructions for your specific OS (Ubuntu/Debian if you're following this guide).
...
10. If you have a while and you want to do Future You a favor, you might as well also install Tidyverse: `install.packages("tidyverse")`. Just note that it takes a while since there's a LOT of Tidyverse packages.
...
- **Installing Snakemake (and also Conda/Mamba):** [https://snakemake.readthedocs.io/en/stable/getting_started/installation.html](https://snakemake.readthedocs.io/en/stable/getting_started/installation.html)
